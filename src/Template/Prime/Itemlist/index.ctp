    ...
    <div class="tab-content content-tab">
      ...
      <div id="tab2" class="tab-pane active content-tab2">
        <div class="content-tab2-title">
          <h1>Primeオリジナル作品一覧(全<?php echo count($item_list) ?>件)</h1>
        </div>
        <div class="content-tab2-elements">
        <?php if(!empty($input_content_list)): ?>
        <div class="content-tab2-element">
          <p>コンテンツ</p>
          <?php for($j=0; $j<count($input_content_list); $j++): ?>
              <button type="button" class="btn btn-secondary"><?php echo $input_content_list[$j]; ?></button>
          <?php endfor; ?>
        </div>
        <?php endif; ?>
