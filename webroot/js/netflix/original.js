...
$(".element-pulldown-topic").on("click", function(){
    var topic = $(this).next();
    var topic_class = topic.attr("class");
        if(topic_class.indexOf("genre-show") != -1){
        $(this).css({"margin-bottom":"0%"});
        topic.removeClass("genre-show");
        topic.hide();
    }else{
        $(this).css({"margin-bottom":"1%"});
        topic.addClass("genre-show");
        topic.show();
    }
});
...
$(".tag").on("click", function(){
  ...
  result = t_arr.indexOf(tname);
  if(result != -1){
    t_arr.splice(result,1);
  }else{
    t_arr.push(tname);
  }
});
...
$("#condition-search").on("click", function(){
    ...
    var t_list = t_arr.join(",");

    $("#select-genre").val(g_list);
    $("#select-tag").val(t_list);
});
